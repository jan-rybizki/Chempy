...
def mcmc(a):
	...
	for i in range(a.m):
		...
		np.save('%s/flatchain' %(directory),np.swapaxes(sampler.get_chain(flat=False),0,1))
		np.save('%s/flatlnprobability' %(directory),np.swapaxes(sampler.get_log_prob(flat=False),0,1))
		np.save('%s/flatblobs' %(directory),np.swapaxes(sampler.get_blobs(flat=False),0,1))
